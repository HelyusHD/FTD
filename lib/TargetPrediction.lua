...
-- Pos                  = where the bullet spawns in global space
...
function TargetPrediction(I,Target,Pos,Vel,g,Mass,Drag,MaxIterationSteps,Accuracy)
    ...
    while (PredictedPosition - PredictedPositionLast).magnitude > Accuracy and Iterations < MaxIterationSteps do
        ...
        Vy = Dy/InterceptionTime - g*InterceptionTime -- Dy = InterceptionTime * Vy + g*InterceptionTime^2
...
function Update(I)
    ...
    local result = TargetPrediction(I,Target,Pos,Vel,g,Mass,Drag,MaxIterationSteps,Accuracy)
